package brickout
{
	...
	public class BrickGame extends MovieClip
	{
		...
		private var dCount:uint = 0;
		private var standalone:Boolean = true;
		private var tourCount:uint = 1;
		private var overlayCount:uint = 2;
		private var sleepTimeX:uint = 10;
		...
		{
			gotoAndPlay(1,"game");
			//this.btnInvert.addEventListener(MouseEvent.CLICK, 
		}
		...
		private function connectStratus():void
		{
			...
			netConnection.addEventListener(AsyncErrorEvent.ASYNC_ERROR,errorEventHandler);
			netConnection.addEventListener(SecurityErrorEvent.SECURITY_ERROR,errorEventHandler);
			netConnection.addEventListener(IOErrorEvent.IO_ERROR,errorEventHandler);
			netConnection.connect(StratusAddress + "/" + DeveloperKey);
		}

		private function errorEventHandler(event:Event):void
		{
			trace("Erreur :"+event.type);
		}

		private function netConnectionHandler(event:NetStatusEvent):void
		{
			trace("Debug count :"+dCount);
